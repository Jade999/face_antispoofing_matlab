...
%
Faces_HSV(12000:2:22000) = [];
labels1(12000:2:22000) = [];
...
Faces_YCbCr(12000:2:22000) = [];
labels2(12000:2:22000) = [];
...
km_centers = vl_kmeans(Data,cluster);
kdtree = vl_kdtreebuild(km_centers);

save('./km_dic.mat','km_centers','kdtree');
...
for i = 1:num_desc:size(Data,2)
   ind = i:i+num_desc-1;
   dataToBeEncoded = Data(:,ind);
   nn = vl_kdtreequery(kdtree,km_centers,dataToBeEncoded);
   assignments = zeros(cluster,num_desc);
   assignments(sub2ind(size(assignments),nn,1:length(nn)))=1;
   Fisher_Data(cnt,:) = vl_vlad(dataToBeEncoded,km_centers,assignments);
...
if strcmp(Classifier,'SVM')
     ...
     save('Model_vlad.mat','Model','Subspace','km_centers','kdtree');
end
