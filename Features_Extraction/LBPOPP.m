...
function Hist = LBPOPP(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.3.3
...
if (nargin == 3) && (length(varargin{3}) == 1)
...
C = image1(origy:origy+dy,origx:origx+dx);
d_C = double(C);
...
Neighboors=zeros(prod(size(result)),neighbors);
for i = 1:neighbors
  y = spoints(i,1)+origy;
  x = spoints(i,2)+origx;
  ...
  if (abs(x - rx) < 1e-6) && (abs(y - ry) < 1e-6)
    ...
    N = w1*d_image2(fy:fy+dy,fx:fx+dx) + w2*d_image2(fy:fy+dy,cx:cx+dx) + ...
        w3*d_image2(cy:cy+dy,fx:fx+dx) + w4*d_image2(cy:cy+dy,cx:cx+dx);
    N = roundn(N,-4);
    D = N >= d_C; 
    Center=d_C;
  end  
  ...
  v = 2^(i-1);
  result = result + v*D;
  Neighboors(:,i)=reshape(N,prod(size(N)),1);
end
 VAR = mean(Neighboors,2);
VAR = reshape(VAR,size(N));
