...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if ((nrhs != 5)&&(nrhs!=7)) {  printf("in nrhs != 5/7 \n"); return; }
...
if (do_subset) {
    for (int c = 0; c<hw; c++) {
        int y = (int) Y[c];
        int x = (int) X[c];
        compute_descriptor(H, params, grid, y, x, shift, desc_out+(c)*DS );
    }
}
else
    for( int y=0; y<h; y++ ) {
    for( int x=0; x<w; x++ ) {
        compute_descriptor(H, params, grid, y, x, shift, desc_out+(x*h + y)*DS );
    }
    }
}
